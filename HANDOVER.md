...
**最后更新**：2026-09-23
**当前阶段**：🤖 **每日自动对账模式** — 40 个模型；价格自动同步，人工只负责新模型的收录判断
...
#### ✅ 最重要的：9/5 的判断被数据证实
Google 发邮件说 28 天 10 次点击。GSC 页面级拆解：

| 页面 | 点击 | 曝光 | CTR | 排名 | 上线 |
|---|---|---|---|---|---|
| `gpt-6-astra` | **4** | 86 | 4.7% | **14.9** | 9/5 |
| `gemini-3-8-flash` | **3** | 44 | 6.8% | **9.9** | 9/5 |
| `gpt-5-6-cyber` | **1** | 11 | 9.1% | **7.3** | 8/24 |
| `/` | 1 | 251 | 0.4% | 77.4 | — |
| 其余全部老页面 | 0 | — | 0% | 40–70 | — |

**9 次点击里 8 次来自最近一个月新上的三个模型页，三个都在第一、二页。** 9/5 写下"没人竞争的新模型能直接进第一页"时是预测，现在是数据。28 天总计：点击 3 → 9，CTR 0.2% → 0.5%，平均排名 57.2 → 48.5（加了页面的情况下均值仍改善）。

仍要说清：绝对量极小，其中一两次可能是用户自己。但三个页面独立地呈现同一形态，且与事先预测一致。

#### 新模型（36 → 40）
自动对账 9/6 起草了 `claude-mythos-5-1`、9/22 起草了 `grok-4-7`，**全程无人介入** —— 自动起草在正常工作。今天补上当天发布的：

| 模型 | 定价 | 看点 |
|---|---|---|
| `claude-opus-5-5` | $4/$20 | 比 Opus 5 便宜 20% |
| `gpt-6-sol` | $2/$10 | |
| `gpt-6-luna` | $0.10/$0.50 | **全站最便宜**（单次 $0.00035），取代 DeepSeek V3.2 |
| `grok-code-fast` | $1/$2 | 8/24 时上游标已退役，现在撤销了；新增 grok 产品线命名规则 |

忽略：`deepseek/deepseek-flash`（与 v4-flash 各字段完全相同，别名）、`xai/grok-code-fast-1`（带版本号别名）。`gpt-5.5-cyber` 上游缺上下文字段 —— 命名规则能推出它，上游补齐后会自动起草，无需人工。`grok-4` 的键被上游删除：它早已标为退役，改为信息提示而非每日警告。

#### 索引提交的缺口
**自动起草会上线页面，但不会提交索引。** `claude-mythos-5-1` 从 9/6 上线到今天没人提交过。今天补交 6 个（4 新 + 这 2 个），逐一确认 GSC 对话框返回"已请求编入索引"。

⚠️ 这一步目前**无法合规地自动化**：Google Indexing API 官方只允许 JobPosting / BroadcastEvent 页面，用于普通页面违反其使用条款；sitemap ping 端点 2023 年已废弃。sitemap 的 lastmod 会自动更新，Google 最终会抓到，手动提交只是加速。**收到 price-sync Issue 里的「已自动上线」时，顺手去 GSC 提交这几个 URL。**

⚠️ 用 JS 批量提交时，一次调用有 45 秒上限，超时后页面内循环仍在跑但结果会丢。**GSC 重新检查一个网址时不会保留"已请求"状态**（用确认成功过的网址校准过），所以事后无法复核 —— 必须一个一个提交、一个一个看对话框。

#### 选择器重做（用户反馈："很不专业"）
40 个模型的下拉框只能一个个翻；顺序就是插入顺序，新模型全掉到最底下。

- `lib/model-order.ts`：按厂商分组（OpenAI → Anthropic → Google → DeepSeek → xAI → Mistral），组内按首次出现日期倒序，退役沉底，30 天内标 New。**排序放展示层**，因为自动起草总往数组末尾追加，改数据文件的顺序会被下一次起草打乱
- 模糊搜索：去标点匹配（`gpt6sol`、`opus55`），整串连读命中给最高权重，子序列只在模型名上匹配。**在真实数据上测出两个问题并修掉**：`gpt 6` 第一个结果曾是 GPT-5.6；`sol` 曾冒出 Claude Opus 5（s-o-l 跨着 opus 和 anthropic 凑出来）
- `components/ModelPicker.tsx`：计算器用，WAI-ARIA combobox，键盘全支持，不引入依赖
- 月度估算器：40 个按钮按厂商分组 + 过滤框
- **浏览器实测**：打开即聚焦搜索框、分组正确、`opus 5.5` + 回车选中后计算器立即变为 $4.00 / 单次 $0.0140、方向键、Esc 不改选择、估算器过滤均正常

**我造成的构建失败**：替换模型下拉框时把 `ui/select` 的 import 整个删了，但货币选择器也在用。事后我只 grep 了"还有没有文件 import ui/select"（当然没有，刚删掉），没查 `<Select` 的用法。Vercel 构建拦住了，生产停在上一个版本，安全失败。**删 import 前先查用法。**

#### 文案随价格自动更新（`scripts/lib/copy-rewrite.mjs`）
9/13 机器人把 deepseek-v4-flash 改成 $0.30/$1.20，文案还写着 $0.44/$1.32，价格表和上面那句话矛盾了十天。自动起草的文案也写死价格，每个草稿第一次降价都会出同样问题。

对账脚本改价时同步改文案，规则刻意收窄：
- **价格对**（`$4.00/$20.00`）在所有模型文案里替换 —— 这是引用兄弟模型的写法，足够具体不会撞
- **单个价格**只在该模型自己的文案里替换，且**不碰紧挨的 `$x/$y`** —— 那是在引用别的模型。第一版没这条，测试里把 `$4/$200` 误改成 `$3/$200`
- **推导出来的说法**（"便宜 20%"、"2.5 倍"）无法机械改写，列入 Issue 的「需复读」

6 个边界用例全过；隔离环境重放 9/13 事故，中英文与 tagline 均正确更新，文案里引用的 V3.2 价格未被误伤。

长期更干净的做法是文案里直接用 `{{price:id}}` 占位符（博客已经这么做），但要改页面、公开 API 和 40 条文案；在本机无法快速类型检查的情况下，先上改动面小、可隔离测试的这一版。
